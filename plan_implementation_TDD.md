...
*   **Principio de Responsabilidad y batchSize = 1:** En la configuración del evento SQS definí `batchSize: 1` *(Archivo: `python-lambdas/serverless.yml`, Bloque: `enviarCorreos -> events -> sqs`)*. Esto significa que la Lambda procesa los mensajes de la cola de a **uno por vez**. Lo dejé así por seguridad y simplicidad: si el procesamiento de un correo falla, solo ese correo regresa a la cola. Si usáramos un lote grande (ej. 10) y uno falla, tendríamos que manejar lógica compleja para evitar reenviar los 9 correos que sí fueron exitosos (control de fallos parciales). Procesar de a 1 garantiza atomicidad y aislamiento de errores. Además, apliqué el **Principio de Mínimo Privilegio (Least Privilege)** en los Roles IAM, otorgando únicamente permisos para `PutItem` y `GetItem` en DynamoDB, asegurando que si una Lambda se ve comprometida, no pueda borrar (`DeleteItem`) ni alterar la estructura de la base de datos completa.
